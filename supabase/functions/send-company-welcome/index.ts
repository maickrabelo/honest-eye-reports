...
Deno.serve(async (req) => {
  ...
  try {
    const { to, companyName, tempPassword, sstName } = await req.json();
    ...
    const result = await resend.emails.send({
      ...
      to: [to],
      subject: `Acesso à plataforma SOIA — ${companyName || "sua empresa"}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #ffffff;">
          <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #666; font-size: 16px;">
              ${sstName ? `A gestora <strong>${sstName}</strong> cadastrou` : "Sua empresa foi cadastrada"}
              ${companyName ? ` <strong>${companyName}</strong>` : ""} na plataforma SOIA.
            </p>
          </div>
          ...
          <div style="background: #f8f9fa; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
            ...
            <p><strong>Senha inicial:</strong> <code style="background:#eee;padding:4px 8px;border-radius:4px;font-size:15px;">${tempPassword}</code></p>
            <p style="color: #dc3545; font-size: 14px;">⚠️ No primeiro acesso será solicitado que você altere a senha.</p>
          </div>
    ...
    console.error("[SEND-COMPANY-WELCOME] Error:", error);
